...
class Emulator:
    def __init__(self, rom_path, q, headless=True, sound=False):
        if headless:
            self.pyboy = PyBoy(
                rom_path,
                window="null",
    ...
    def press_buttons(self, buttons, wait=True):
        ...
        Args:
            buttons (list[str] or str): List of buttons or single button to press
        ...
        # Check if buttons is a string, and if so, convert to a list
        ...
        for button in buttons:
            ...
            print("Done pressing button. Waiting:")
            if wait:
